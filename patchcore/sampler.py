...
class GreedyCoresetSampler(BaseSampler):
    ...
    def run_with_limit_memory(
        ...
        """Subsamples features using Greedy Coreset with memory limit constraints.

        Args:
            features: [N x D]
            memory_size: Maximum allowed samples in the memory bank.
        """
        if self.percentage == 1:
            return features
        self._store_type(features)
    ...
    def _compute_greedy_coreset_indices_with_limit_memory(self, features: torch.Tensor, memory_size) -> np.ndarray:
        """Runs iterative greedy coreset selection constrained by memory size.

        Args:
            features: [NxD] input feature bank to sample.
            memory_size: Maximum allowed samples.
        """
        distance_matrix = self._compute_batchwise_differences(features, features)
        coreset_anchor_distances = torch.norm(distance_matrix, dim=1)
...
class ApproximateGreedyCoresetSampler(GreedyCoresetSampler):
    ...
    def _compute_greedy_coreset_indices(self, features: torch.Tensor) -> np.ndarray:
        ...
        with torch.no_grad():
            for _ in tqdm.tqdm(range(num_coreset_samples), desc="Coreset Subsampling..."):
    ...
    def _compute_greedy_coreset_indices_with_limit_memory(self, features: torch.Tensor, memory_size) -> np.ndarray:
        """Runs approximate iterative greedy coreset selection constrained by memory size.
        ...
        with torch.no_grad():
            for _ in tqdm.tqdm(range(num_coreset_samples), desc="Coreset Subsampling (Memory Limited)..."):
                select_idx = torch.argmax(approximate_coreset_anchor_distances).item()
                coreset_indices.append(select_idx)
                coreset_select_distance = self._compute_batchwise_differences(
                    features, features[select_idx: select_idx + 1]  # noqa: E203
